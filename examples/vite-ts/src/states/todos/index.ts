...
import type { Selector } from '@raulpesilva/re-state/types/react/types';
...
export const todoInitialValue: TodoList = [];
...
export const resetTodo = (): void => dispatchTodo(todoInitialValue);
export const getTodoById = (id: string): Todo | undefined => getTodo().find((todo) => todo.id === id);
export const addTodo = ({ task, id }: { task: Todo['task']; id?: Todo['id'] }): void =>
  dispatchTodo((prev) => [...prev, { task, id: id ?? Date.now().toString(), finished: false }]);
export const removeTodo = (id: string): void => dispatchTodo((prev) => prev.filter((todo) => todo.id !== id));
export const toggleTodo = (id: Todo['id']): void =>
