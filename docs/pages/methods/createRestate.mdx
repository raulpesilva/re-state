...
```ts filename="states/volume.ts"
import { createReState } from '@raulpesilva/re-state';

export const useVolume = createReState<number>('volume', 50);
```

```tsx filename="components/VolumeSlider.tsx"
...
```tsx filename="components/VolumeDisplay.tsx"
import { useVolume } from '../states/volume';
...
```tsx filename="components/UserCard.tsx"
...
```tsx filename="components/AudioControls.tsx"
